        ...
        for line in file:
            if not trobat:
                if line.replace("\n", "") == "":
                    ...
                    dict[line.replace("\n", "").split(":")[0].strip()] =  line.replace("\n", "").split(":")[1].strip()
            elif trobat:
                if line.replace("\n", "") != "":
                    guardar_linea.append(line)

        while len(guardar_linea) > 0:
            for line in guardar_linea:
                if line.replace("\n", "").split(" ")[0].strip() in dict and line.replace("\n", "").split(" ")[2].strip() in dict:
                    guardar_linea.remove(line)
                    operation = line.replace("\n", "").split(" ")[1].strip()
                    valor1 = dict[line.replace("\n", "").split(" ")[0].strip()]
                    valor2 = dict[line.replace("\n", "").split(" ")[2].strip()]

                    guarda = line.replace("\n", "").split("->")[1].strip()
                    if operation == "AND":
                        if valor1 == "1" and valor2 == "1":
                            dict_resulat[guarda] = "1"
                            dict[guarda] = "1"
                        else:
                            dict_resulat[guarda] = "0"
                            dict[guarda] = "0"
                    elif operation == "OR":
                        ...
                        if valor1 == "1" and valor2 == "0" or valor1 == "0" and valor2 == "1":
                            dict_resulat[guarda] = "1"
                            dict[guarda] = "1"
                        else:
                            dict_resulat[guarda] = "0"
                            dict[guarda] = "0"
    keys = sorted(dict_resulat.keys())
    resultat = ""
    for key in keys:
        if key[0] == "z":
            resultat = dict_resulat[key] + resultat
